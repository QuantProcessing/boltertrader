...
func (s *snapshotAccount) Positions(context.Context) ([]model.Position, error) { return s.positions, nil }
func (s *snapshotAccount) SetLeverage(context.Context, model.InstrumentID, decimal.Decimal) error {
	return nil
}
func (s *snapshotAccount) SetMarginMode(context.Context, model.InstrumentID, string) error { return nil }
...
func TestReconcileOrders(t *testing.T) {
	...
	if rep.OrdersUpdated != 1 || rep.OrdersExternal != 1 || rep.OrdersCleared != 1 {
		t.Fatalf("report=%+v, want updated=1 external=1 cleared=1", rep)
	}
	...
	// Order "b" cleared to Canceled (no longer resting at venue).
	if o, ok := c.Order("b"); !ok || o.Status != enums.StatusCanceled {
		t.Fatalf("order b not cleared: ok=%v status=%v", ok, o.Status)
	}
...
func TestReconcileOrdersNilExec(t *testing.T) {
	...
	if rep.OrdersUpdated+rep.OrdersExternal+rep.OrdersCleared != 0 {
