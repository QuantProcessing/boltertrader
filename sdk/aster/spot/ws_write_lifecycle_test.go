...
func TestWSClientCloseInterruptsBlockedWrite(t *testing.T) {
	conn, writeObserved := newBlockedSpotWebSocket(t)
	...
	waitForSpotBlockedWrite(t, writeObserved)
...
func TestWSClientBlockedStaleWriteDoesNotCloseReplacement(t *testing.T) {
	stale, writeObserved := newBlockedSpotWebSocket(t)
...
func waitForSpotBlockedWrite(t *testing.T, observed <-chan struct{}) {
	t.Helper()
	select {
	case <-observed:
	case <-time.After(time.Second):
...
func newBlockedSpotWebSocket(t *testing.T) (*websocket.Conn, <-chan struct{}) {
	t.Helper()
	...
	go func() {
		...
		accept := spotWebSocketAccept(request.Header.Get("Sec-WebSocket-Key"))
...
func spotWebSocketAccept(key string) string {
