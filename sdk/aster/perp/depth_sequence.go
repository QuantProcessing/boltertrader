...
func (s *DepthSequence) Accept(event WsDepthEvent) (bool, error) {
	...
	if event.FirstUpdateID <= 0 || event.FinalUpdateID < event.FirstUpdateID {
		return false, newDepthSequenceGapError(s.lastUpdateID+1, event)
	}
	...
	if event.FirstUpdateID > expected || event.FinalUpdateID < expected ||
		...
		return false, newDepthSequenceGapError(expected, event)
	}
...
func newDepthSequenceGapError(expected int64, event WsDepthEvent) *DepthSequenceGapError {
	return &DepthSequenceGapError{
		expected:      expected,
...
func (e *DepthSequenceGapError) Error() string {
	return fmt.Sprintf("aster perp depth sequence gap: expected update %d, received [%d,%d]", e.expected, e.firstUpdateID, e.finalUpdateID)
}
