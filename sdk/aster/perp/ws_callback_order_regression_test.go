...
func TestPerpWsReplacementDataWaitsForRecoveredCallback(t *testing.T) {
	t.Setenv("PROXY", "")
	...
	wantPerpCallbackEvent(t, events, "old")
	releaseFirst.Do(func() { close(allowFirstClose) })
	wantPerpCallbackEvent(t, events, "started")
...
func wantPerpCallbackEvent(t *testing.T, events <-chan string, want string) {
	t.Helper()
	select {
	case got := <-events:
		if got != want {
