...
func TestSignedRESTDebugLogRedactsSignatureAndResponseToken(t *testing.T) {
	const responseToken = "binance-spot-listen-key-secret"
	...
	if err := client.Get(context.Background(), "/api/v3/account", map[string]any{"symbol": "BTCUSDT"}, true, &result); err != nil {
		t.Fatalf("signed Get: %v", err)
	}

	logged := binanceSpotObservedText(observed.All())
...
func TestWSDebugRequestSummaryRedactsCredentialsAndSignature(t *testing.T) {
	const secret = "binance-spot-ws-signature-secret"
	summary := wsDebugRequestSummary(map[string]any{
		"apiKey":    "binance-spot-ws-api-key-secret",
		"signature": secret,
	})
...
func TestWSDebugResponseLogRedactsTokens(t *testing.T) {
	const token = "binance-spot-ws-listen-key-secret"
	core, observed := observer.New(zap.DebugLevel)
	...
	if logged := binanceSpotObservedText(observed.All()); strings.Contains(logged, token) || strings.Contains(logged, "listenKey") {
...
func TestSignedRESTTransportErrorRedactsNestedRequestSignature(t *testing.T) {
	const signingSecret = "binance-spot-signing-secret"
	...
	client.HTTPClient = &http.Client{Transport: binanceSpotRoundTripFunc(func(req *http.Request) (*http.Response, error) {
	...
	err := client.Get(context.Background(), "/api/v3/account", map[string]any{"symbol": "BTCUSDT"}, true, nil)
	if err == nil {
...
type binanceSpotRoundTripFunc func(*http.Request) (*http.Response, error)

func (fn binanceSpotRoundTripFunc) RoundTrip(req *http.Request) (*http.Response, error) {
	return fn(req)
}

func binanceSpotObservedText(entries []observer.LoggedEntry) string {
