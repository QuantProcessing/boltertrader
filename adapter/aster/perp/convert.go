...
func mapAsterCommandError(err error) error {
	...
	case -1013, -1111, -1121, -2010, -2011, -2013, -2019, -2021, -2022, -4164:
...
func validateSubmitOrderResponseIdentity(r *sdkperp.OrderResponse, req model.OrderRequest, inst *model.Instrument) error {
	if r == nil || inst == nil {
		return fmt.Errorf("aster perp: submit response and instrument are required")
	}
	if strings.TrimSpace(r.Symbol) == "" || r.Symbol != inst.VenueSymbol {
		return fmt.Errorf("aster perp: submit response symbol mismatch: requested %q, got %q", inst.VenueSymbol, r.Symbol)
	}
	if strings.TrimSpace(r.ClientOrderID) == "" || r.ClientOrderID != req.ClientID {
		return fmt.Errorf("aster perp: submit response client order id mismatch: requested %q, got %q", req.ClientID, r.ClientOrderID)
	}
	if r.OrderID <= 0 {
		return fmt.Errorf("aster perp: submit response venue order id must be positive, got %d", r.OrderID)
	}
	return nil
}
