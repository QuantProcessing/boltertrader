...
func TestSpotMassStatusRetainsSuccessfulRowsWhenAnotherInstrumentFails(t *testing.T) {
	first := mustSpotInstrument(t)
	second := *first
	...
	client.WithHTTPClient(&http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		body := `[{"symbol":"ASTERUSDT","orderId":42,"clientOrderId":"retained-spot","status":"NEW","type":"LIMIT","side":"BUY","timeInForce":"GTC","origQty":"1","price":"10","executedQty":"0","cumQty":"0","cumQuote":"0","updateTime":1700000000000}]`
...
func TestSpotMassStatusFreezesSelectorAndEarliestRequestStart(t *testing.T) {
...
func TestSpotMassStatusDistinguishesEmptyFromPreIOUnavailable(t *testing.T) {
	...
	failedClient := spotClientNoNetwork(t)
...
func TestSpotMassStatusRejectsMismatchedScopeBeforeIO(t *testing.T) {
	called := false
	client := spotClientNoNetwork(t)
	client.WithHTTPClient(&http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		called = true
