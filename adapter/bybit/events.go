...
func execEventsFromOrderMessage(msg *bybitsdk.WSOrderMessage, resolve func(string) model.InstrumentID) []contract.ExecEvent {
	if msg == nil {
		return nil
	}
	...
	for _, record := range msg.Data {
		...
		out = append(out, contract.OrderEvent{Order: orderFromBybitRecord(record, id, AccountIDUnified)})
	}
	return out
}

func execEventsFromExecutionMessage(msg *bybitsdk.WSExecutionMessage, resolve func(string) model.InstrumentID) []contract.ExecEvent {
	if msg == nil {
		return nil
	}
	out := make([]contract.ExecEvent, 0, len(msg.Data))
...
func accountEventsFromPositionMessage(msg *bybitsdk.WSPositionMessage, resolve func(string) model.InstrumentID, now time.Time) []contract.AccountEvent {
	if msg == nil {
		return nil
	}
	out := make([]contract.AccountEvent, 0, len(msg.Data))
	for _, record := range msg.Data {
		pos := positionFromBybit(record, resolve, AccountIDUnified, now)
		out = append(out, contract.PositionEvent{Position: pos})
	}
	return out
}

func accountEventsFromWalletMessage(msg *bybitsdk.WSWalletMessage, now time.Time) []contract.AccountEvent {
	if msg == nil {
		return nil
	}
