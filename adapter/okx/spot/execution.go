...
// derivative-only and is intentionally absent from the Spot inventory.
var spotPendingAlgoOrderTypes = [...]string{
...
func (c *executionClient) GenerateExecutionMassStatus(ctx context.Context, query model.MassStatusQuery) (*model.ExecutionMassStatus, error) {
	...
	algoPages := c.fetchPendingAlgoPages(ctx, instTypeSpot)
	instType := instTypeSpot
...
func (c *executionClient) fetchPendingAlgoPages(ctx context.Context, instType string) []pendingAlgoPage {
	pages := make([]pendingAlgoPage, 0, len(spotPendingAlgoOrderTypes))
	for _, orderType := range spotPendingAlgoOrderTypes {
