...
func TestOKXSpotPrivateReconnectEmitsGapEnvelopes(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for {
			...
			case "subscribe":
				...
				if connection == 1 && subscriptions == 1 {
	...
	exec := newExecutionClient(nil, provider, clock.NewRealClock(), defaultSpotTdMode, "OKX:test")
	acct := newAccountClient(nil, provider, clock.NewRealClock(), "OKX:test")
	adapter := &Adapter{
		Execution:    exec,
		Account:      acct,
		provider:     provider,
	...
	assertOKXSpotGapEnvelope(t, exec.Events(), contract.StreamGapStarted, time.Second)
	assertOKXSpotGapEnvelope(t, exec.Events(), contract.StreamGapRecovered, 8*time.Second)
	if got := connections.Load(); got != 2 {
		t.Fatalf("websocket connections=%d, want 2", got)
	}
}

func assertOKXSpotGapEnvelope(t *testing.T, events <-chan contract.ExecEnvelope, phase contract.StreamGapPhase, timeout time.Duration) {
	t.Helper()
	select {
	case envelope := <-events:
		...
		if event.Venue != venueName || event.AccountID != "OKX:test" || event.StreamID != "okx:spot:private" || event.Generation != 1 || event.Phase != phase {
