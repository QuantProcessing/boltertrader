...
// instrumentFromOKX translates one OKX SWAP Instrument into a neutral
// Instrument. The OKX InstIdCode populates VenueIntCode — the divergence the
// contract test verifies. Returns nil for non-live or non-SWAP entries.
func instrumentFromOKX(in *okx.Instrument) *model.Instrument {
	if in.InstType != instTypeSwap {
		return nil
	}
