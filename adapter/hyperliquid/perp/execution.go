...
func (c *executionClient) placeOrderRequest(req model.OrderRequest, venueCloid *string) (sdkperp.PlaceOrderRequest, error) {
...
func (c *executionClient) GenerateExecutionMassStatus(ctx context.Context, query model.MassStatusQuery) (*model.ExecutionMassStatus, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("hyperliquid perp: mass status account: %w", err)
	}
	if venue := strings.TrimSpace(query.Venue); venue != "" && venue != venueName {
		return nil, fmt.Errorf("hyperliquid perp: mass status venue %q does not match %q", query.Venue, venueName)
	}
	...
	if len(insts) == 0 {
		...
		return validateMassStatusResult(mass, query, "hyperliquid perp")
	}
	if c.rest == nil {
	...
	for _, inst := range insts {
		...
		dex := ""
		if value, _, found := strings.Cut(inst.VenueSymbol, ":"); found {
			dex = value
		}
		dexSet[dex] = struct{}{}
	}
	frozenRegistry := instruments.NewRegistry(insts...)
	dexes := make([]string, 0, len(dexSet))
	...
	for _, dex := range dexes {
		...
		for i := range orders {
			...
			order := orderFromHL(&orders[i], frozenRegistry, accountID)
			order.Request.InstrumentID = id
			order = c.rememberOrder(order)
...
func (c *executionClient) massStatusInstruments(requested []model.InstrumentID) ([]*model.Instrument, []model.InstrumentID, error) {
	if c.provider == nil {
		return nil, nil, fmt.Errorf("hyperliquid perp: instrument provider required for mass status")
	}
	...
	for _, id := range model.NormalizeInstrumentIDs(candidates) {
		if id.Venue != venueName || id.Kind != enums.KindPerp || id.Symbol == "" {
			return nil, nil, fmt.Errorf("hyperliquid perp: mass status instrument %s is outside execution scope", id)
		}
		inst, ok := c.provider.Instrument(id)
		if !ok || inst == nil {
			return nil, nil, fmt.Errorf("hyperliquid perp: unknown mass status instrument %s", id)
		}
