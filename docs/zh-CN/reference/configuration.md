...
| Lighter Testnet | `LIGHTER_TESTNET_SPOT_SYMBOL=ETH-USDC`; `LIGHTER_TESTNET_PERP_SYMBOL=ETH-USDC` | `LIGHTER_TESTNET_MAX_NOTIONAL_USDC=100` |
...
- Binance 选择内置 Demo REST/WS profile，不提供 acceptance endpoint override
  variable。
- Bybit 选择内置 **Demo Trading** profile。`BYBIT_TESTNET_API_KEY` 与
  `BYBIT_TESTNET_API_SECRET` 代表另一个环境，明确禁止用它们替代 Demo
  credential。
